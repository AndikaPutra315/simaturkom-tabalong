...
<head>
    ...
    <title>Kelola Data Bakti - Admin</title>
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="content-card">
                <div class="card-header">
                    <h1>Kelola Data Bakti</h1>
                    <p>Tambah, edit, atau hapus data infrastruktur Bakti di Kabupaten Tabalong.</p>
                </div>
                <div class="toolbar">
                    <form action="{{ route('suadmin.databakti.index') }}" method="GET" class="filter-group">
                    ...
                    <div class="action-buttons">
                        <a href="{{ route('suadmin.databakti.index') }}" class="btn-custom btn-refresh">
                            <i class="fas fa-sync-alt"></i> Refresh
                        </a>
                        <a href="{{ route('suadmin.databakti.pdf', request()->query()) }}" class="btn-custom btn-pdf" target="_blank">
                        ...
                        <a href="{{ route('suadmin.databakti.create') }}" class="btn-custom btn-add">
                            ...
                            @forelse ($dataBakti as $item)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="status-badge status-nonaktif">{{ $item->status ?? 'N/A' }}</span>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        ...
                                        <form action="{{ route('suadmin.databakti.destroy', $item->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form action="{{ route('suadmin.databakti.destroy', $item->id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="10" class="text-center py-5">
                                        ...
                                        <p>Coba ubah filter Anda atau <a href="{{ route('suadmin.databakti.create') }}">tambahkan data baru</a>.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div class="modal fade" id="importExcelModal" tabindex="-1" aria-labelledby="importExcelModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="importExcelModalLabel">Upload Data Excel (Data Bakti)</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('suadmin.databakti.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <p>Pilih file Excel (.xlsx atau .xls) untuk diimpor. Pastikan urutan dan format kolom sudah sesuai template.</p>

                        <div class="mb-3">
                            <a href="{{ asset('files/contoh template data bakti.xlsx') }}" class="btn btn-outline-success btn-sm" download>
